...
function on_ws_msg(data)
{
  ...
  if (msg == "Library Updated")
   ...
   {
    playing = false
    Array.from(document.getElementsByClassName('time_total')).forEach(el => { el.innerHTML  = "00:00:00"; });    
    Array.from(document.getElementsByClassName('time_elapsed')).forEach(el => { el.innerHTML  = "00:00:00"; });
...
function after_CurrentTrack()
{
  ...
  Array.from(document.getElementsByClassName('time_elapsed')).forEach(el => { el.innerHTML  =hhmmssposition; });
  Array.from(document.getElementsByClassName('time_total')).forEach(el => { el.innerHTML  = MsToMMSS(responseObject.duration); });
